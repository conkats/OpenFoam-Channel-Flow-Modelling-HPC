...
:
    ...
    implicitDiv_(coeffDict_.lookupOrAddDefault<Switch>("implicitDiv", true)),

    SSG_(coeffDict_.lookupOrAddDefault<Switch>("SSG", true)),
    Cg1_
    (
        dimensioned<scalar>::lookupOrAddToDict
        (
...
//.............................................................
    ...
    KolmogorovFlagStart_(coeffDict_.lookupOrAddDefault<scalar>("KolmogorovFlagStart", 0)),
    KolmogorovFlagEnd_(coeffDict_.lookupOrAddDefault<scalar>("KolmogorovFlagEnd", 0)),
...
//    oneField oneFieldloc;

    KolmogorovFlag_ = ((runTime_.value()>=KolmogorovFlagStart_ && runTime_.value()<=KolmogorovFlagEnd_) ? pos(yStarLim_ - y/ystar) :  ystar/(ystar+ySMALL_));
...
tmp<fvVectorMatrix> GenElliptic::divDevReff(volVectorField& U) const
{
    if(implicitDiv_)
    {
        return
            (
                fvc::div(R_)
                + fvc::laplacian(nutIMP(), U)
                - fvm::laplacian(nu(), U)
                - fvm::laplacian(nutIMP(), U)
            );
    }
    else
    {
...
tmp<fvVectorMatrix> GenElliptic::divDevRhoReff
(
